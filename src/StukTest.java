...
public class StukTest {
    ...
    @Test
    public void updateMogelijkeZettenKoningin() {
        Speelveld veld = new Speelveld();
        veld.getSTUKKEN()[4][3] = new Koningin(true, 4, 3);

        System.out.println(veld.getSTUKKEN()[4][3].updateMogelijkeZetten(veld));
        System.out.println();
        veld.printVeld();

        assert veld.getSTUKKEN()[4][3].updateMogelijkeZetten(veld).size() == 27;
    }

    @Test
    public void pionTestTweePlekkenMogelijkOpEersteRij() {
        Speelveld veld = new Speelveld();
        veld.creeerStukken();
        assert veld.getSTUKKEN()[6][0].updateMogelijkeZetten(veld).size() == 2;
        Speelveld veld2 = veld.move(new Zet(6, 0, 5, 0));
